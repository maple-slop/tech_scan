...
from .fetchers.browser import AsyncBrowserPool, BrowserSession, fetch_browser, fetch_browser_async
...
class ScanRunner:
    def __init__(
        ...
        browser_session: BrowserSession | AsyncBrowserPool | None = None,
    ):
        self.args = args
        ...
        with ResponseCache(self.args.db) as cache:
            async def cached_or_fetch(mode: str) -> FetchResult:
                ...
                if not self.args.refresh:
                    ...
                    self.diagnostics.log(3, f"cache bypass refresh: target={target} mode={mode}")
                sanity = await asyncio.to_thread(
                    check_target_ports,
                    raw_target,
                    target,
                    getattr(self.args, "sanity_timeout", 1.0),
                ...
                if mode == "browser":
                    browser_pool = (
                        self.browser_session
                        if isinstance(self.browser_session, AsyncBrowserPool)
                        else None
                    )
                    ...
                    fresh_fetch = await asyncio.to_thread(
                        fetch_requests,
                        raw_target,
                        target,
                        self.args.timeout,
